...
describe("Validate all questionanires", () => {
    ...
    test("All questionnaires are valid", async () => {
        const questionnaires = extractBundleResources(ensure(await getFHIRResources<Questionnaire>('Questionnaire', {_count: 9999}))).Questionnaire;
        for(let q of questionnaires){
            ...
            const outcome = ensure(await service<OperationOutcome>({url: '/Questionnaire/$validate', data: q, method: 'POST'}))
